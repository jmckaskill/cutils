...
static int poll_add_socket(const async_class **vt, async_socket *s, int fd, async_socket_cb cb) {
	async_poll *a = (async_poll*)vt;
	...
	size_t i = a->num++;
	a->sockets[i] = s;
	a->pfd[i].events = POLLHUP | POLLERR;
	a->pfd[i].fd = fd;
...
static ssize_t poll_read(const async_class **vt, async_socket *s, char *buf, size_t len) {
	...
	size_t i = s->idx;
	assert(i < a->num && a->sockets[i] == s);
	int r = recv(s->fd, buf, len > INT_MAX ? INT_MAX : (int)len, 0);
	if (r < 0 && would_block()) {
...
static ssize_t poll_write(const async_class **vt, async_socket *s, const char *buf, size_t len) {
	async_poll *a = (async_poll*)vt;
	size_t i = s->idx;
	assert(i < a->num && a->sockets[i] == s);
